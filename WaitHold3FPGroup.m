...
tablenames = {'ANM','Strain', 'Task', 'Treatment',...
    'nTrial','Accuracy','SSRT_median','HoldDur_median','ST_median',...
    'PerfTotal','PerfPort1','PerfPort2','Perf_shortFP','Perf_midFP','Perf_longFP',...
    'SSRT_Port','SRT_FP','HoldDur_Port','HoldDur_FP'};
ATF_ANM=[];ATF_Stn=[];ATF_Task=[];ATF_Treatment=[];ATF_nTrial=[];ATF_Acc=[];ATF_SRT=[];ATF_HoldDur=[];ATF_ST=[];
PerfTotal=[]; PerfPort1=[];PerfPort2=[];Perf_shortFP=[];Perf_midFP=[];Perf_longFP=[];
SRT_Port=[];SRT_FP=[];HoldDur_Port=[];HoldDur_FP=[];
...
for i = 1: length(ANM_all)  
    ...
    ATF_SRT=[ATF_SRT; compute_stat_summary(Temp.SRT(Temp.OutcomeCode==1 & Temp.Treatment==0),'quartile')'; compute_stat_summary(Temp.SRT(Temp.OutcomeCode==1 & Temp.Treatment==1),'quartile')']; % no warmup; only correct
    
    ATF_HoldDur=[ATF_HoldDur;median(Temp.HoldDur(Temp.Treatment==0),'omitnan'); median(Temp.HoldDur(Temp.Treatment>0),'omitnan')];
    ATF_ST=[ATF_ST; median(rmoutliers(Temp.ST(Temp.Treatment==0)),'omitnan'); median(rmoutliers(Temp.ST(Temp.Treatment>0)),'omitnan')];

    PerfTotal=[PerfTotal; [sum(Temp.OutcomeCode(Temp.Treatment==0)>-2), sum(Temp.OutcomeCode==1 & Temp.Treatment==0),sum(Temp.OutcomeCode==-1 & Temp.Treatment==0),sum(Temp.OutcomeCode==0 & Temp.Treatment==0)];
                          [sum(Temp.OutcomeCode(Temp.Treatment>0)>-2), sum(Temp.OutcomeCode==1 & Temp.Treatment>0),sum(Temp.OutcomeCode==-1 & Temp.Treatment>0),sum(Temp.OutcomeCode==0 & Temp.Treatment>0)]  ]; 
    PerfPort1=[PerfPort1; [sum(Temp.OutcomeCode(Temp.CorPort==1 & Temp.Treatment==0)>-2),sum(Temp.OutcomeCode(Temp.CorPort==1 & Temp.Treatment==0)==1),sum(Temp.OutcomeCode(Temp.CorPort==1 & Temp.Treatment==0)==-1),sum(Temp.OutcomeCode(Temp.CorPort==1 & Temp.Treatment==0))==0];
                          [sum(Temp.OutcomeCode(Temp.CorPort==1 & Temp.Treatment>0)>-2),sum(Temp.OutcomeCode(Temp.CorPort==1 & Temp.Treatment>0)==1),sum(Temp.OutcomeCode(Temp.CorPort==1 & Temp.Treatment>0)==-1),sum(Temp.OutcomeCode(Temp.CorPort==1 & Temp.Treatment>0)==0)]];
    PerfPort2=[PerfPort2; [sum(Temp.OutcomeCode(Temp.CorPort==2 & Temp.Treatment==0)>-2),sum(Temp.OutcomeCode(Temp.CorPort==2 & Temp.Treatment==0)==1),sum(Temp.OutcomeCode(Temp.CorPort==2 & Temp.Treatment==0)==-1),sum(Temp.OutcomeCode(Temp.CorPort==2 & Temp.Treatment==0)==0)];
                          [sum(Temp.OutcomeCode(Temp.CorPort==2 & Temp.Treatment>0)>-2),sum(Temp.OutcomeCode(Temp.CorPort==2 & Temp.Treatment>0)==1),sum(Temp.OutcomeCode(Temp.CorPort==2 & Temp.Treatment>0)==-1),sum(Temp.OutcomeCode(Temp.CorPort==2 & Temp.Treatment>0)==0)]];
    
    Perf_shortFP=[Perf_shortFP; [sum(Temp.OutcomeCode(Temp.FP==0.5 & Temp.Treatment==0)>-2),sum(Temp.OutcomeCode(Temp.FP==0.5 & Temp.Treatment==0)==1),sum(Temp.OutcomeCode(Temp.FP==0.5 & Temp.Treatment==0)==-1),sum(Temp.OutcomeCode(Temp.FP==0.5 & Temp.Treatment==0)==0)];
                                [sum(Temp.OutcomeCode(Temp.FP==0.5 & Temp.Treatment>0)>-2),sum(Temp.OutcomeCode(Temp.FP==0.5 & Temp.Treatment>0)==1),sum(Temp.OutcomeCode(Temp.FP==0.5 & Temp.Treatment>0)==-1),sum(Temp.OutcomeCode(Temp.FP==0.5 & Temp.Treatment>0)==0)]];
    Perf_midFP=[Perf_midFP; [sum(Temp.OutcomeCode(Temp.FP==1 & Temp.Treatment==0)>-2),sum(Temp.OutcomeCode(Temp.FP==1 & Temp.Treatment==0)==1),sum(Temp.OutcomeCode(Temp.FP==1 & Temp.Treatment==0)==-1),sum(Temp.OutcomeCode(Temp.FP==1 & Temp.Treatment==0)==0)];
                            [sum(Temp.OutcomeCode(Temp.FP==1 & Temp.Treatment>0)>-2),sum(Temp.OutcomeCode(Temp.FP==1 & Temp.Treatment>0)==1),sum(Temp.OutcomeCode(Temp.FP==1 & Temp.Treatment>0)==-1),sum(Temp.OutcomeCode(Temp.FP==1 & Temp.Treatment>0)==0)]];
    Perf_longFP=[Perf_longFP; [sum(Temp.OutcomeCode(Temp.FP==1.5 & Temp.Treatment==0)>-2),sum(Temp.OutcomeCode(Temp.FP==1.5 & Temp.Treatment==0)==1),sum(Temp.OutcomeCode(Temp.FP==1.5 & Temp.Treatment==0)==-1),sum(Temp.OutcomeCode(Temp.FP==1.5 & Temp.Treatment==0)==0)];
                              [sum(Temp.OutcomeCode(Temp.FP==1.5 & Temp.Treatment>0)>-2),sum(Temp.OutcomeCode(Temp.FP==1.5 & Temp.Treatment>0)==1),sum(Temp.OutcomeCode(Temp.FP==1.5 & Temp.Treatment>0)==-1),sum(Temp.OutcomeCode(Temp.FP==1.5 & Temp.Treatment>0)==0)]];
    
    SRT_Port=[SRT_Port;compute_stat_summary(Temp.SRT(Temp.OutcomeCode==1 & Temp.CorPort==1 & Temp.Treatment==0),'quartile')', compute_stat_summary(Temp.SRT(Temp.OutcomeCode==1 & Temp.CorPort==2 & Temp.Treatment==0),'quartile')';
                    compute_stat_summary(Temp.SRT(Temp.OutcomeCode==1 & Temp.CorPort==1 & Temp.Treatment>0),'quartile')', compute_stat_summary(Temp.SRT(Temp.OutcomeCode==1 & Temp.CorPort==2 & Temp.Treatment>0),'quartile')'];
    SRT_FP=[SRT_FP;compute_stat_summary(Temp.SRT(Temp.OutcomeCode==1 & Temp.FP==0.5 & Temp.Treatment==0),'quartile')', compute_stat_summary(Temp.SRT(Temp.OutcomeCode==1 & Temp.FP==1 & Temp.Treatment==0),'quartile')', compute_stat_summary(Temp.SRT(Temp.OutcomeCode==1 & Temp.FP==1.5 & Temp.Treatment==0),'quartile')';
                 compute_stat_summary(Temp.SRT(Temp.OutcomeCode==1 & Temp.FP==0.5 & Temp.Treatment>0),'quartile')', compute_stat_summary(Temp.SRT(Temp.OutcomeCode==1 & Temp.FP==1 & Temp.Treatment>0),'quartile')', compute_stat_summary(Temp.SRT(Temp.OutcomeCode==1 & Temp.FP==1.5 & Temp.Treatment>0),'quartile')'];
    
    HoldDur_Port=[HoldDur_Port;median(Temp.HoldDur(Temp.OutcomeCode==1 & Temp.CorPort==1 & Temp.Treatment==0),'omitnan'), median(Temp.HoldDur(Temp.OutcomeCode==1 & Temp.CorPort==2 & Temp.Treatment==0),'omitnan');
                               median(Temp.HoldDur(Temp.OutcomeCode==1 & Temp.CorPort==1 & Temp.Treatment>0),'omitnan'), median(Temp.SRT(Temp.OutcomeCode==1 & Temp.CorPort==2 & Temp.Treatment>0),'omitnan')];
    HoldDur_FP=[HoldDur_FP;median(Temp.HoldDur(Temp.OutcomeCode==1 & Temp.FP==0.5 & Temp.Treatment==0),'omitnan'), median(Temp.HoldDur(Temp.OutcomeCode==1 & Temp.FP==1 & Temp.Treatment==0),'omitnan'), median(Temp.HoldDur(Temp.OutcomeCode==1 & Temp.FP==1.5 & Temp.Treatment==0),'omitnan');
                           median(Temp.HoldDur(Temp.OutcomeCode==1 & Temp.FP==0.5 & Temp.Treatment>0),'omitnan'), median(Temp.HoldDur(Temp.OutcomeCode==1 & Temp.FP==1 & Temp.Treatment>0),'omitnan'), median(Temp.HoldDur(Temp.OutcomeCode==1 & Temp.FP==1.5 & Temp.Treatment>0),'omitnan')];

end
clear Temp
ATF=table(ATF_ANM,ATF_Stn,ATF_Task,ATF_Treatment, ...
    ATF_nTrial,ATF_Acc,ATF_SRT,ATF_HoldDur,ATF_ST, ...
    PerfTotal, PerfPort1,PerfPort2,Perf_shortFP,Perf_midFP,Perf_longFP,...
    SRT_Port,SRT_FP,HoldDur_Port,HoldDur_FP,...
...
clear ATF_ANM ATF_Stn ATF_Date ATF_Task ATF_HoldDur ATF_Treatment ATF_nTrial ATF_ToC ATF_Acc ATF_Pre ATF_SSRT ATF_ST ATF_Bpodfiles ATF_Csvfiles PerfTotal PerfPort1 PerfPort2 Perf_shortFP Perf_midFP Perf_longFP SSRT_Port SSRT_FP HoldDur_Port HoldDur_FP
...
perf_FP=[ATF.Perf_shortFP(ATF.Treatment==0,2:4)./ATF.Perf_shortFP(ATF.Treatment==0,1),ATF.Perf_shortFP(ATF.Treatment==1,2:4)./ATF.Perf_shortFP(ATF.Treatment==1,1)];
perf_FP_Ebar=[];
...
HoldDur=DataUsed.HoldDur;
% figure()
% set(gcf,'unit', 'centimeters', 'position',[1 1 13 8], 'paperpositionmode', 'auto' );
for j=1:length(FPtype)
    axes('Units', 'centimeter', 'Position', [1+3.5*(j-1) 4.5 2.5 2.5],'FontSize',6); hold on;
    xlim([0,3]);xlabel('Hold duration(s)');xticks([0:0.5:4]);xtickangle(0);
    ylim([0,1]);ylabel('CDF');
    title(['FP=',sprintf('%.1f',FPtype(j)),'s']);
    Acdf_Saline=[];Acdf_DCZ=[];cdf_Saline=[];cdf_DCZ=[];
    for i =1:length(ANM_all)-1
    
        Acdf_Saline(i,:)= ksdensity([HoldDur(strcmp(DataUsed.Subject,ANM_all(i)) & DataUsed.FP==FPtype(j) & DataUsed.Treatment==0) ], timebins,'Support','positive',...
            'Function','cdf','Bandwidth',0.05);
        Acdf_DCZ(i,:) = ksdensity([HoldDur(strcmp(DataUsed.Subject,ANM_all(i)) & DataUsed.FP==FPtype(j) & DataUsed.Treatment==1)], timebins,'Support','positive',...
            'Function','cdf','Bandwidth',0.05);
    end
...
for j=1:length(FPtype)
    ...
    ylim([0,1]);ylabel('CDF');
    title(['FP=',sprintf('%.1f',FPtype(j)),'s']);    
    cdf_Saline= ksdensity([HoldDur(Data_Sample.FP==FPtype(j) & Data_Sample.Treatment==0) ], timebins,'Support','positive',...
        'Function','cdf','Bandwidth',0.05);
    cdf_DCZ= ksdensity([HoldDur(Data_Sample.FP==FPtype(j) & Data_Sample.Treatment==1)], timebins,'Support','positive',...
        'Function','cdf','Bandwidth',0.05);
    plot(timebins, cdf_Saline(1,:), '-', 'Color',  [0 0 0], 'linewidth', 1);hold on;
    plot(timebins, cdf_DCZ(1,:), '-', 'Color',  cDCZ, 'linewidth', 1);hold on;
    plot([FPtype(j),FPtype(j)],[0 1],'--','Color','k','LineWidth',.7);hold on;
...
violinplot(ST(DataSTG.Stn==1), cellstr(string(TreatmentST(DataSTG.Stn==1))), 'ViolinColor', [.5 .5 .5; .5 .5 .5], ...
    'ShowMedian', false, 'ShowWhiskers', false, 'ShowBox', false, 'ShowData', false);
for i =1:length(ANM_BN)

    scatter(1, median(ST(strcmp(DataSTG.Subject,ANM_BN(i)) & TreatmentST==0), 'omitnan'), 20, 'MarkerFaceColor', 'none', 'MarkerEdgeColor', [.3 .3 .3], 'MarkerEdgeAlpha', 0.7, 'LineWidth', 1.2);
    scatter(2, median(ST(strcmp(DataSTG.Subject,ANM_BN(i)) & TreatmentST>0), 'omitnan'), 20, 'MarkerFaceColor', 'none', 'MarkerEdgeColor', [.3 .3 .3], 'MarkerEdgeAlpha', 0.7, 'LineWidth', 1.2);
end
ylim([0.5 15]); xlim([.5 2.5]);
ylabel('ShuttleTime / s', 'FontSize', 8);
xticklabels({'Saline', 'DCZ'}); xlabel('Treatment', 'FontSize', 8);
